...
export class PostgresQueryRepository implements IQueryRepository {
  ...
  async findShipmentWithCheckpoints(trackingId: string): Promise<{ shipment: Shipment; checkpoints: Checkpoint[] } | null> {
    const shipmentResult = await this.pool.query('SELECT * FROM shipments WHERE tracking_id = $1', [trackingId]);
    if (shipmentResult.rowCount === 0) {
      return null;
    }
    const shipmentRow = shipmentResult.rows[0];
    const shipment: Shipment = { // Mapeo de snake_case a camelCase
      id: shipmentRow.id,
  ...
  async findShipmentsByStatus(status: string, page: number, limit: number): Promise<{ shipments: Shipment[]; total: number }> {
    const offset = (page - 1) * limit;

    const totalResult = await this.pool.query('SELECT COUNT(id) FROM shipments WHERE current_status = $1', [status]);
